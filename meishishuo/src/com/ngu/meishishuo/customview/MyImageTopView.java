package com.ngu.meishishuo.customview;
...
public class MyImageTopView extends ViewGroup {//自定义控件，从ViewGroup继承而来
	...
	private ImageView[] topImageViews,bottomImageViews;//上方图片，下方圆点
	...
	public void init() {
		...
		timer.schedule(new TimerTask() {//定时操作，每个8秒发送消息			
			public void run() {
	...
	public void scrollToImage(int targetIndex){//跳转到目标图片
		...
		resetImg();
		//改变下方圆圈的状态
		bottomImageViews[currentImageIndex].setImageResource(R.drawable.circle_choosed);			
	}	
	...
	public void initImages(int[] imgIds) {// 初始化显示的图片
		...
		topImageViews=new ImageView[num];
	...
	public void setBottomImageViews(ImageView[] imgViews){
		this.bottomImageViews=imgViews;
	}
	public void resetImg() {// 该方法用于将所有的圆圈都显示为未选中状态
		for (int i = 0; i < bottomImageViews.length; i++) {
			bottomImageViews[i].setImageResource(R.drawable.circle_unchoosed);
		}
	}
}
